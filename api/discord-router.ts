...
export const discordRouter = createRouter({
  ...
  stock: publicQuery.query(async () => {
    ...
    const nitro = await db
      ...
      .from(schema.discordUsers)
      .where(and(
        eq(schema.discordUsers.status, "active"),
        eq(schema.discordUsers.nitro, "yes")
      ));
  ...
  checkServer: publicQuery.input(z.object({
    ...
    try {
      const res = await axios.get(
        `https://discord.com/api/v10/guilds/${input.serverId}`,
        {
          headers: {
    ...
    for (const user of users) {
      try {
        ...
        const res = await axios.put(
          `https://discord.com/api/v10/guilds/${input.serverId}/members/${user.discordId}`,
